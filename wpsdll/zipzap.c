...
VOID _Optlink unzipit(PVOID n);
INT showZZip(CHAR *zname, CHAR *intzname);
INT viewZipOnDClickInt(CHAR *fqzf, CHAR *zf);
VOID  _Optlink insertContTH(PVOID n);
VOID  _Optlink insertContXTH(PVOID n);
VOID _Optlink handleDrop(PVOID n);
...
CHAR exeicon[40];
CHAR fileicon[40];
HPOINTER hptr, hptrs, hptrfold, hptrfile, hptrexe, hptrzip;
INT rtries;
HWND hwndGDlg;
                                             ...
                                             ULONG param)
{
    ...
    if(_somIsA(somSelf, _WPDesktop))
       ...
       if( OPENONCE == 0 )
          ...
          hptrfold = WinLoadPointer(HWND_DESKTOP, hResource, ID_ICONFOLD);
          hptrfile = WinLoadPointer(HWND_DESKTOP, hResource, ID_ICONFILE);
          hptrzip   = WinLoadPointer(HWND_DESKTOP, hResource, ID_ICONZIP);
          hptrexe   = WinLoadPointer(HWND_DESKTOP, hResource, ID_ICONEXE);
          /*
          hptrfold = WinLoadFileIcon("h:\\OS2", FALSE);
          hptrfile = WinLoadFileIcon("h:\\OS2\\E.EXE", FALSE);
          hptrzip  = WinLoadFileIcon("h:\\aazz\\zipf.ico", FALSE);
          hptrexe  = WinLoadFileIcon("h:\\OS2\\CMD.EXE", FALSE);
          */
          }
       OPENONCE++;
       }
...
strcpy(FQZIPNAME, fqzf);
   ...
   // debugMsgStr(pgmInputs, "pgmInputs");
   ...
   rcq = DosStartSession(&StartData, &SessID, &PID);
...
switch ( msg )
   ...
   case WM_INITDLG:
      ...
      WinSetOwner(WinWindowFromID(hwndDlg, ID_CONTOBJ), hwndDlg);
      ...
      WinSendDlgItemMsg(hwndDlg, ID_CONTOBJ, CM_SETCNRINFO, 0,0);
 
      hwndGDlg = hwndDlg;

      if( levelnum == 0 )
         // insertCont(hwndDlg, TFNAME, ZIPNAME);
         _beginthread(insertContTH, NULL, ZZSTACKSIZE, (PVOID)hwndDlg);
      else   
         _beginthread(insertContXTH, NULL, ZZSTACKSIZE, (PVOID)levelnum);
            ...
            switch (SHORT2FROMMP(mp1)) 
               ...
               case CN_INITDRAG:
                  ...
                  if( pcnrdraginit->pRecord )
                     ...
                     prec = (PMAIN_1000_REC)pcnrdraginit->pRecord;
                     if( prec )
                        {
                        // _beginthread(unzipFile, NULL, ZZSTACKSIZE, (PVOID)somSelf);
                        // unzipFile(prec->fqname);
                        if( strstr(prec->fqname, "/") )
                           {
                           strcpy(dfn, prec->fqname);
                           for( i=0;i<strlen(dfn);i++ )
                              {
                              if( dfn[i] == '/' )
                                 x = i;
                              }
                  ...
                  drgitem.hstrContainerName = DrgAddStrHandle("h:\\aazz");
                  drgitem.hstrSourceName = DrgAddStrHandle(dfn);
                  drgitem.hstrTargetName = DrgAddStrHandle(dfn);
                  drgitem.cxOffset = 0 ;
                  drgitem.cyOffset = 0 ;
               ...
               case CN_ENTER:
                  ...
                  if(pNotRecEnter->pRecord)
                     ...
                     prec = (PMAIN_1000_REC)pNotRecEnter->pRecord;
                     ss = strstr(prec->Record.pszIcon, "bytes]");
                     if( !ss )
                        {
                        viewOnContDClick(hwndDlg, TFNAME, ZIPNAME, prec->Record.pszIcon, lastlevelnum+1);
                        }
                     else
                        {
                        if( strcmpi(prec->fqname + (strlen(prec->fqname) - 4), ".ZIP") == 0 )
                           {
                           showZZip(FQZIPNAME, prec->fqname);
                           }
                        else 
                           {  
                           if( strcmpi(prec->fqname + (strlen(prec->fqname) - 4), ".EXE") != 0 && 
                               strcmpi(prec->fqname + (strlen(prec->fqname) - 4), ".COM") != 0 &&
                               strcmpi(prec->fqname + (strlen(prec->fqname) - 4), ".DLL") != 0 )
                              { 
                              // viewFile();
                              debugMsgStr(prec->fqname, "prec->fqname");
                              }
                           }
                        }   
                     }
...
CHAR pgmInputs[ZZMAXPATH];
APIRET rcq;
CHAR targetZip[ZZMAXPATH];
INT tr;
HOBJECT hob;


_getcwd(tmpPath, sizeof(tmpPath));
strcpy(pgmInputs, "/C unzip.exe -o -j ");
strcat(pgmInputs, "\"");
strcat(pgmInputs, zname);
strcat(pgmInputs, "\"");
strcat(pgmInputs, " ");
strcat(pgmInputs, "\"");
strcat(pgmInputs, intzname);
strcat(pgmInputs, "\"");
strcat(pgmInputs, " -d ");
strcat(pgmInputs, tmpPath);
...
if (rcq != 0)
   {
   msgBox(HWND_DESKTOP, "Attention!", "Session failed!");
   return(-1);
   }
   
strcpy(targetZip, tmpPath);
if( targetZip[strlen(targetZip)-1] != '\\' )
   strcat(targetZip, "\\");
strcat(targetZip, intzname);

tr = 0;
do
   {
   DosSleep(100);
   hob = WinQueryObject(targetZip);
   tr++;
   }
while( hob == NULLHANDLE && tr < NUMTRIES );   

// debugMsgStr(targetZip, "targetZip");

viewZipOnDClickInt(targetZip, intzname);
...
wio = _wpclsQueryObjectFromFrame(_WPDesktop, hwndT);
_wpQueryRealName(wio, rpath, &rpathsize, TRUE);
msgBox(HWND_DESKTOP, "rpath", rpath);
...
do
   {
   DosSleep(100);
   rc = DosOpen(TFNAME, &fhan, &act, 0L, 0L,
...
while(rc > 0 && rtries < NUMTRIES );                


if (rc != 0 )
   {
   DosClose(fhan);
   msgBox(HWND_DESKTOP, "Failed to open temp file!", TFNAME);
   }
else
   {
   DOIT = FALSE;
         ...
         if( result )
            ...
            if( NEWONE )
               ...
               pRecord=(PVOID)WinSendDlgItemMsg(hwndDlg, ID_CONTOBJ,
                                                                   CM_ALLOCRECORD,
                                                                   MPFROMLONG(ZZMAXPATH+sizeof(*pRecord)-sizeof(MINIRECORDCORE)+32), 
                                                                   MPFROMLONG(1));                  
               pRecord->Record.cb=sizeof(MINIRECORDCORE);
               pRecord->Record.flRecordAttr=0;

               // pRecord->Record.hptrIcon=WinLoadPointer(HWND_DESKTOP, hResource, ID_ICONFOLD);
               pRecord->Record.hptrIcon=hptrfold;
               pRecord->Record.pszIcon=(PBYTE)pRecord+sizeof(*pRecord);
        
               strcpy(pRecord->Record.pszIcon, tmpname);
               pRecord->fqname=(PBYTE)pRecord+ZZMAXPATH;
               // pRecord->fqname=NULL;
            ...
            if( nlen > 4 )
               ...
               if( strcmpi(name + (strlen(name) - 4), ".EXE") == 0  ||
                  ...
                  {
                  // pRecord->Record.hptrIcon=WinLoadPointer(HWND_DESKTOP, hResource, ID_ICONEXE);
                  pRecord->Record.hptrIcon=hptrexe;
                  }
               else
                  {
                  if( strcmpi(name + (strlen(name) - 4), ".ZIP") == 0 )
                     // pRecord->Record.hptrIcon=WinLoadPointer(HWND_DESKTOP, hResource, ID_ICONZIP);
                     pRecord->Record.hptrIcon=hptrzip;
                  else    
                     // pRecord->Record.hptrIcon=WinLoadPointer(HWND_DESKTOP, hResource, ID_ICONFILE);
                     pRecord->Record.hptrIcon=hptrfile;
                  }    
               }
            else
               {
...
// INT insertContXTH(HWND hwndDlg, CHAR *fname, CHAR *zname, INT level)
VOID _Optlink insertContXTH(PVOID n)
{
HFILE fhan, fhanTMP;
APIRET rc, rcTMP;
...
if (rc != 0 )
   {
   DosClose(fhan);
   debugMsgStr(TFNAME, "Failed to open temp file!");
   }
else
   {
...
if (rc != 0 )
   {
   DosClose(fhan);
   msgBox(HWND_DESKTOP, "Failed to open temp file!", fname);
   }
else
   {
   DOIT = FALSE;
...
WinSetWindowText(WinWindowFromID(hwndDlg, FID_TITLEBAR), "loading zipfile data ...");
strcpy(fileicon, "filef.ico");
strcpy(exeicon, "exef.ico");
strcpy(zipicon, "zipf.ico");
strcpy(foldicon, "foldf.ico");
...
if (rc != 0 )
   {
   DosClose(fhan);
   debugMsgStr(fname, "Failed to open temp file!");
   }
else
   {
   fnlen = strlen(FOLDNAME);
...
if ( (MYHINI = PrfOpenProfile(0, ZZINI )) != NULLHANDLE )
   ...
   if ( rsizeX == (sizeof(pgmData)) )
      ...
      pgmData[24] = 1;
      pgmData[25] = 0;
      pgmData[26] = 0;  /* NOT IN USE  VERBOSE OUTPUT VIEWER */
      pgmData[27] = 1;
      pgmData[28] = 2;
      pgmData[29] = 1;
      pgmData[30] = 1;
